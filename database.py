...
def init_db():
    """Initialize Supabase tables via SQL if they don't exist."""
    sb = get_client()
    # Tables are created in Supabase dashboard via SQL editor
    # Just ensure default settings exist
    try:
...
def get_categories():
    try:
        sb  = get_client()
        res = sb.table("categories").select("name").order("name").execute()
...
def ingest_excel(df, mode="add_new"):
    ...
    # Get existing barcodes for add_new mode
    existing_barcodes = set()
    if mode == "add_new":
        res = sb.table("transactions").select("barcode").execute()
...
def get_dashboard_stats():
    ...
    res = sb.table("transactions").select("status").execute()
    rows = res.data or []
    status_counts = {}
    for r in rows:
    ...
    res2 = sb.table("transactions").select("date_ingested,status,date_actioned,days_to_action,category,auditor").execute()
